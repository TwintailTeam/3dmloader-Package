...
fn has_3dmigoto_description(data: &[u8], module_path: &str) -> bool {
    ...
    let count = size as usize / size_of::<LangCodePage>();
    for i in 0..count {
        let lc = unsafe { &*ptr.add(i) };
        let sub = std::ffi::CString::new(format!("\\StringFileInfo\\{:04x}{:04x}\\FileDescription", lc.language, lc.code_page)).unwrap();
...
pub fn check_3dmigoto_version(module_path: &str) {
    let path_c = std::ffi::CString::new(module_path).expect("module path contains null byte");
